...
- **✅✅ ÉTAT FINAL v3 (retour user + diagnostic empirique) : image générée À LA DEMANDE CÔTÉ SERVEUR (edge Cloudflare).** La génération sur l'appareil (v2) montrait le **logo au lieu du pari** dans l'unfurl. Diagnostic empirique (banc de test navigateur + revue adversariale) : (a) la capture web (`html-to-image`/`foreignObject`) est **fragile multi-navigateurs** (échoue sur Safari iOS, cale en onglet arrière-plan car elle attend une frame `requestAnimationFrame`) ; (b) **course temporelle** — l'utilisateur pouvait partager avant la fin de l'upload → le crawler récupérait le logo, **mis en cache par WhatsApp/FB** ; (c) régression trouvée : attendre la capture avant `navigator.share()` **consommait l'activation du geste** → la feuille de partage ne s'ouvrait plus (web). **Solution retenue (choix user) :** `functions/og/bet/[id].ts` = image PNG **rendue à la demande** via `workers-og` (Satori+resvg sur l'edge), thème Noir **fidèle à la ShareCard** (wordmark, drapeaux inlinés best-effort en data-URL, score + t.a.b., combiné à pastilles vert/rouge, gain sauge/rouge, pseudo), noms d'équipes en FR via `frTeam`. `functions/s/bet/[id].ts` : `og:image = /og/bet/:id` (l'endpoint retombe lui-même sur `/og-image.jpg` en cas d'erreur → jamais de 500/logo cassé) + redirige l'humain vers `/partage/:id`. `onShare` = partage **synchrone** du lien (préserve l'activation web). **Capture sur l'appareil retirée** : suppression de `ShareCard.tsx`, `shareCard.ts`, mutations `generateShareUploadUrl`/`setShareImage`, `shareImageUrl` (queries) et champ schéma `shareImageId` ; deps `-html-to-image -react-native-view-shot +workers-og`. **Validé en vrai runtime Cloudflare** (`wrangler pages dev`) sur paris gagné/perdu réels : `/og/bet/:id` → `200 image/png` fidèle ; `/s/bet/:id` pointe bien `og:image` vers l'endpoint. Toujours l'aperçu du pari, même partage immédiat, iPhone compris.
- (v2 obsolète) **✅ ÉTAT FINAL (retour PO) : image pré-générée par l'app → stockée dans Convex → servie comme OG.** L'app compose la **ShareCard fidèle** (Noir, drapeaux+score+combiné+gain sauge+pseudo) **hors-écran**, la capture (`captureCardBlob` : html-to-image sur web / react-native-view-shot en natif, **split runtime `Platform.OS`** + imports dynamiques — pas d'extension `.web` car peu fiable avec l'alias `@/`), l'**upload dans Convex storage** (`bets.generateShareUploadUrl` + `uploadToConvex` + `bets.setShareImage`), sur l'écran résultat d'un pari réglé si pas déjà générée. `bets.publicShare`/`byId` renvoient `shareImageUrl` ; schéma `bets.shareImageId`. La **génération edge workers-og a été RETIRÉE** (plus de wasm serveur). `functions/s/bet/[id].ts` : `og:image = shareImageUrl ?? og-image.jpg` et **redirige l'humain vers `/partage/:id`** (page PUBLIQUE hors auth, `src/app/partage/[id].tsx`, via `publicShare` + CTA inscription) — corrige le « Pari introuvable » du destinataire non‑proprio (byId est owner-only). Partage = **lien** `/s/bet/:id` via `shareLink`.
- Historique des tentatives (obsolètes) :
- **⚠️ PIVOT (retour PO) : passage à l'OG dynamique SERVEUR** (l'« alternative » documentée), la **capture d'écran a été retirée**. Le partage envoie un **LIEN** `/s/bet/:id` (via `shareLink`) qui **s'unfurl** avec une carte perso générée côté serveur — plus de capture utilisateur. Nouvelle implémentation : (1) `convex/bets.publicShare` = query publique (id non devinable, données d'affichage seulement) ; (2) **Cloudflare Pages Function** `functions/s/bet/[id].ts` = HTML avec balises OG par pari + redirect humain vers `/bet/:id` ; (3) `functions/og/bet/[id].ts` = **image PNG dynamique** via `workers-og` (Satori+resvg sur l'edge Cloudflare), thème Noir, polices depuis `/fonts/`. Revue adversariale → corrigé : séparateur de milliers **U+202F** (absent de Sora → tofu) remplacé par une espace ASCII ; garde `res.ok` sur les polices + **dégradation 302 vers `/og-image.jpg`** (jamais de 500 au crawler). Deps : `-react-native-view-shot -html-to-image -expo-sharing`, `+workers-og +@cloudflare/workers-types`. `functions/` exclu du tsc app (typé via `functions/tsconfig.json`).
- **À valider au déploiement** (non testable en local) : routing Functions vs SPA `_redirects` (`/s/bet/:id` et `/og/bet/:id` doivent être servis par les Functions — Cloudflare auto‑génère `_routes.json`) ; `EXPO_PUBLIC_CONVEX_URL` présent comme variable d'env Pages (runtime des Functions) ; taille du bundle Worker (resvg wasm) sous la limite du plan ; unfurl via les validateurs Twitter/Facebook + WhatsApp. Déployer via l'**intégration Git** (build Cloudflare, qui prend `functions/` à la racine) — PAS `wrangler pages deploy dist` (ne copie pas `functions/`).

--- Historique (V1 abandonnée) : partage d'IMAGE par capture, sans serveur. Split plateforme `shareImage.ts` (natif : `react-native-view-shot` + `expo-sharing`) / `shareImage.web.ts` (web : `html-to-image` + Web Share Files / téléchargement) → chaque bundle n'embarque que sa lib.
